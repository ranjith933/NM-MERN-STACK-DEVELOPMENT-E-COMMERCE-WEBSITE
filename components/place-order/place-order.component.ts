...
export class PlaceOrderComponent {
  ...
  constructor(private http: HttpClient, private route: Router, private activatedRoute: ActivatedRoute) {
    ...
    this.searchText = '';
    this.http.get<any[]>('http://localhost:4200/products').subscribe(data => {
      this.data = data;
    });
  ...
  onAddToCart(productId: string): void {
    this.isLoading = true;
    this.http.post('http://localhost:4200/add-to-cart', { "productId": productId }).subscribe((res) => {
      if (res) {
  ...
  createOrder(orderDetails = {firstname:String,lastname:String, phone: String, productId: this.routerId, address: String, quantity: String, paymentMethod: String }): void {
    ...
    console.log(order)
    this.http.post('http://localhost:4200/orders',order).subscribe((response) => {
